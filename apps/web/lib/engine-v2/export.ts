...
import { resolveToken } from "./document.ts";
...
function nodeHtml(node: EngineNode, tokens: EngineTokens): string {
  ...
  if (node.type === "path") return `<svg data-node-id="${escapeMarkup(node.id)}" viewBox="0 0 ${Math.max(...node.points.map((point) => point.x), 1)} ${Math.max(...node.points.map((point) => point.y), 1)}"${styleAttribute}><${node.closed ? "polygon" : "polyline"} points="${node.points.map((point) => `${point.x},${point.y}`).join(" ")}" fill="${node.closed ? escapeMarkup(resolveToken(node.style?.background, tokens) ?? "transparent") : "none"}" stroke="${escapeMarkup(resolveToken(node.style?.borderColor ?? node.style?.color, tokens) ?? color(tokens, "ink"))}" stroke-width="${node.style?.borderWidth ?? 3}" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
  if (node.type === "metric") {
