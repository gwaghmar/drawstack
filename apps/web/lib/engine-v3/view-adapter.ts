...
function nodeFromRecord(record: ResolvedRenderRecord, parent: ResolvedRenderRecord | undefined, children: EngineNode[]): EngineNode {
  ...
  if (record.type === "frame") return { ...base, type: "frame", children } as EngineNode;
  return base as EngineNode;
}
...
export function createEngineV3PageView(document: EngineDocumentV3, pageId: string): EngineDocument {
  ...
  const build = (record: ResolvedRenderRecord): EngineNode => nodeFromRecord(record, record.parentId ? recordsById.get(record.parentId) : undefined, (byParent.get(record.id) ?? []).map(build));
