...
function standaloneSvg(document: EngineDocumentV3, page: Page): string {
  ...
  const text = (value: string, x: number, y: number, size: number, colorValue: string, weight = 400, anchor: "start" | "middle" | "end" = "start", style: { fontStyle?: unknown; textDecoration?: unknown } = {}) => `<text x="${x}" y="${y}" text-anchor="${anchor}" font-family="Inter,ui-sans-serif,system-ui,sans-serif" font-size="${size}" font-weight="${weight}"${style.fontStyle === "italic" ? " font-style=\"italic\"" : ""}${style.textDecoration === "underline" ? " text-decoration=\"underline\"" : ""} fill="${colorValue}">${value.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;")}</text>`;
