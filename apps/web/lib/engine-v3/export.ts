...
function standaloneSvg(document: EngineDocumentV3, page: Page): string {
  ...
  const markup = records.filter((record) => record.id !== page.root.id && record.visible).map((record) => {
    ...
    if (record.type === "path") {
      ...
      return `<g data-node-id="${record.id}" transform="${transform}"${opacity}>${defs}<path d="${closed ? `${lineStyle} Z` : lineStyle}" fill="${closed ? fill(record) : "none"}" stroke="${stroke(record) === "#D7DBD2" ? textColor(record) : stroke(record)}" stroke-width="${Number(record.style?.borderWidth ?? 3)}"${dash} stroke-linecap="${record.style?.strokeLinecap ?? "round"}" stroke-linejoin="${record.style?.strokeLinejoin ?? "round"}"${marker}/></g>`;
    }
