...
function standaloneSvg(document: EngineDocumentV3, page: Page): string {
  ...
  const text = (value: string, x: number, y: number, size: number, colorValue: string, weight = 400, anchor: "start" | "middle" | "end" = "start", style: { fontFamily?: unknown; fontStyle?: unknown; textDecoration?: unknown; letterSpacing?: unknown } = {}) => `<text x="${x}" y="${y}" text-anchor="${anchor}" font-family="${typeof style.fontFamily === "string" ? style.fontFamily : "Inter,ui-sans-serif,system-ui,sans-serif"}" font-size="${size}" font-weight="${weight}"${style.fontStyle === "italic" ? " font-style=\"italic\"" : ""}${style.textDecoration === "underline" ? " text-decoration=\"underline\"" : ""}${style.letterSpacing !== undefined ? ` letter-spacing=\"${Number(style.letterSpacing)}px\"` : ""} fill="${colorValue}">${value.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;")}</text>`;
  const markup = records.filter((record) => record.id !== page.root.id && record.visible).map((record) => {
    ...
    if (record.type === "text") {
      ...
      const fontWeight = typeof source.style === "object" && source.style && "fontWeight" in source.style && (typeof source.style.fontWeight === "number" || typeof source.style.fontWeight === "string") ? source.style.fontWeight : variant === "body" ? 400 : 650;
      const textStyle = typeof source.style === "object" && source.style ? source.style : {};
      return `<g data-node-id="${record.id}" transform="${transform}"${opacity}>${text(String(source.content ?? ""), textX, size, size, textColor(record), Number(fontWeight), anchor, textStyle)}</g>`;
    }
    ...
    if (record.type === "path") {
      ...
      const first = rawPoints[0] ?? { x: 0, y: 0 }; const last = rawPoints.at(-1) ?? first;
      const lineStyle = source.lineStyle === "curve" ? `M ${first.x} ${first.y} C ${(first.x + last.x) / 2} ${first.y}, ${(first.x + last.x) / 2} ${last.y}, ${last.x} ${last.y}` : source.lineStyle === "elbow" ? `M ${first.x} ${first.y} L ${(first.x + last.x) / 2} ${first.y} L ${(first.x + last.x) / 2} ${last.y} L ${last.x} ${last.y}` : `M ${rawPoints.map((point) => `${point.x} ${point.y}`).join(" L ")}`;
      ...
      const dash = record.style?.borderStyle === "dashed" ? ` stroke-dasharray="10 8"` : record.style?.borderStyle === "dotted" ? ` stroke-dasharray="2 6"` : "";
      return `<g data-node-id="${record.id}" transform="${transform}"${opacity}>${defs}<path d="${closed ? `${lineStyle} Z` : lineStyle}" fill="${closed ? fill(record) : "none"}" stroke="${stroke(record) === "#D7DBD2" ? textColor(record) : stroke(record)}" stroke-width="${Number(record.style?.borderWidth ?? 3)}"${dash} stroke-linecap="round" stroke-linejoin="round"${marker}/></g>`;
    }
