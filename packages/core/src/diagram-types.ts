...
3. System Architecture, Process Maps & HUDs:
   ...
   - any shape: "shadow": true adds a soft drop shadow (cards/tables/dashboards already have one); "shadow": false removes it. Geometric primitives are flat by default.
   - "arrow" | "line": { "start": {"shapeId":"id1","anchor":"auto"}, "end": {"shapeId":"id2","anchor":"auto"}, "routing": "orthogonal" | "curved" | "straight", "label": "...", "waypoints": [{"x":300,"y":150}], "showJunctions": true } — waypoints bend the connector through intermediate points; showJunctions draws small ring markers at each point — use for routed/dense diagrams with visible junctions.
     ...
     ERD cardinality (crow's foot) — set one on EACH end, the head describes the entity it points at: "crowfoot-one" (exactly one) | "crowfoot-many" (many) | "crowfoot-zero-one" (optional one) | "crowfoot-one-many" (one or more) | "crowfoot-zero-many" (zero or more). Example one-to-many: { "type": "line", "start": {"shapeId":"customers"}, "end": {"shapeId":"orders"}, "arrowHeadStart": "crowfoot-one", "arrowHeadEnd": "crowfoot-many" }. Pair with the "table" shape for the entity boxes.
